...
public class ClientUserRegistController {
     ...
     * 
     * @param form ユーザーフォーム
     * @return "redirect:/client/user/regist/input" 新規会員登録画面　表示処理
     */
	@RequestMapping(path = "/client/user/regist/input/init",method = RequestMethod.GET)
	public String link(@ModelAttribute UserForm form) {
		UserForm userForm = new UserForm();
		session.setAttribute("userForm", userForm);
		return "redirect:/client/user/regist/input";
	}
	/**
	 * 2入力画面　表示処理(POST) 一覧画面での新規ボタン押下後の処理
	 * 
	...
	public String buttonfur(@ModelAttribute UserForm form,Model model) {
		...
		// 入力画面　表示
		return "/client/user/regist_input"; //変える
	}
	...
	public String checkfur(Model model) {
		//セッションから入力フォーム情報取得
				UserForm userForm = (UserForm) session.getAttribute("userForm");
				if (userForm == null) {
					// セッション情報がない場合、エラー
